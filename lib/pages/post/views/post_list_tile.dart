...
class PostListTile<C extends PostActionMixin> extends GetView<C>
    with LoadImageMixin {
  final Post post;
  final space = const SizedBox(height: 5);
  final ValueSetter<Post> onTap;

  PostListTile({Key? key, required this.post, required this.onTap})
      : super(key: key);
  ...
  Widget get _headerItem =>
      GetBuilder<C>(id: post.id, builder: (_) => PostListHeader<C>(post: post));
  ...
  Widget get _photosItem => PostPhotosTile(
      photos: post.medias,
      onTap: (index) => controller.pushToMediaPage(post.medias, index));
